...
import static com.fbm.util.Randoms.num;
 ...
 * @version:1.0
 */
public class ImageUtils extends Captcha {
	...
	public ImageUtils(int width, int height, int len) {
		this(width, height);
		this.len = len;
		str="";
	}

	public ImageUtils(int width, int height, int len, Font font) {
	...
	private boolean graphicsImage(char[] strs, OutputStream out) {
		...
		try {
			...
			for (int i = 0; i < len; i++) {
				...
				g.drawString(c + "", (width - (len - i) * w) + size, h - 4);
